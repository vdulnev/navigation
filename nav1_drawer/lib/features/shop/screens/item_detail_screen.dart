...
///
/// • **No [Drawer]** — detail screens are nested within a section. The drawer
///   belongs only to section root screens. The AppBar shows a Back arrow
///   instead.
///
/// • **`ModalRoute.settings.arguments`** — the [Product] is passed via
///   `pushNamed(AppRoutes.shopDetail, arguments: product)` and read here.
///
/// • **Plain `pushNamed` auth guard** — no `rootNavigator: true` needed;
///   the single Navigator is always reachable.
class ItemDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // No drawer — sub-screens navigate back via the AppBar back button.
      body: ListView(
        ...
        children: [
          ...
          FilledButton.icon(
            ...
            onPressed: () async {
              if (!AuthService.isLoggedIn) {
                // Plain pushNamed — single Navigator, no rootNavigator: true.
                final loggedIn =
          ...
          const SizedBox(height: 24),
          const NavNoteCard(
            title: 'Navigator 1: arguments + plain pushNamed auth guard',
            body:
                'Product passed via pushNamed arguments, read with '
                'ModalRoute.of(context).settings.arguments. '
                'Login uses plain pushNamed — no rootNavigator: true.',
          ),
        ],
      ),
    );
