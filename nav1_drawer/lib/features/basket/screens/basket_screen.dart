...
///
/// • **Auth gate with plain `pushNamed`** — no `rootNavigator: true` required.
///   `setState()` rebuilds this screen when the login route pops.
///
/// • **`pushNamed` within section** — the Checkout button pushes
///   `/basket/checkout` on the single Navigator; back returns here.
  ...
  Widget build(BuildContext context) {
    if (!AuthService.isLoggedIn) {
      return Scaffold(
        ...
        drawer: const AppDrawer(activeRoute: AppRoutes.basket),
        body: Center(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 24),
                const NavNoteCard(
                  title: 'Navigator 1: auth gate, no rootNavigator',
                  body:
                      'pushNamed(/login) targets the single root Navigator '
                      'directly. setState() rebuilds this screen when login '
                      'pops — no state management library needed.',
                ),
              ],
            ),
          ),
